...
try {
    ...
    // 1. Lấy danh sách đơn chưa phân công
    $sql = "SELECT id, id_benhvien, ngayhen, giohen FROM datdichvu WHERE ngayhen >= CURDATE() + INTERVAL 1 DAY
              AND ngayhen <= CURDATE() + INTERVAL 3 DAY
              AND trangthai = '0'  ";
    $dondichvus = $conn->query($sql)->fetchAll(PDO::FETCH_ASSOC);
    $lich_chua_pc = [];
    $lich_phan_cong = [];
    $don_khong_phan_cong = [];
    ...
    foreach ($dondichvus as $don) {
        ...
        $stmt->execute([$benhvien_id, $ngayhen]);
        $nhanvien = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($nhanvien) {
            // 3. Ghi vào bảng phân công và cập nhật trạng thái
            $stmt = $conn->prepare("UPDATE datdichvu SET id_nhanvien = ?, trangthai = '1' WHERE id = ?");
            $stmt->execute([$nhanvien['id'], $don['id']]);
            ...
            // Lưu các đơn không thể phân công
            $don_khong_phan_cong[] = [
                'id_don' => $don['id'],
                'id_benhvien' => $don['id_benhvien'],
                'ngayhen' => $don['ngayhen']
    ...
    echo json_encode([
        ...
        'data' => [
            'lich_phan_cong' => $lich_phan_cong,
            'don_khong_phan_cong' => $don_khong_phan_cong,
