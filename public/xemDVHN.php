...
<script>
        ...
        // Hàm gọi API bắt đầu dịch vụ
        function batDauDichVu(orderId) {
            fetch('http://localhost/WEB_ThueHoTroKhamBenh/api/batdauDVAPI.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: orderId })
            })
            .then(response => response.text()) // đổi tạm từ .json()
.then(text => {
    ...
    try {
        ...
        showNotification('Phản hồi API không đúng định dạng JSON.', 'danger');
    }
})

        }

        // Hàm gọi API kết thúc dịch vụ
        function ketThucDichVu(orderId) {
            fetch('http://localhost/WEB_ThueHoTroKhamBenh/api/ketthucDVAPI.php', {
                ...
                .then(data => {
                    if (data.success) {
                        showNotification(data.message, 'success');
                        //fetchOrders('xemDVHNAPI.php'); // Tải lại danh sách đơn hàng
                    } else {
                        showNotification(data.message, 'danger');
                    }
                })
    ...
    $(document).ready(function () {
        ...
        function fetchOrders(apiEndpoint) {
            ...
            fetch(`http://localhost/WEB_ThueHoTroKhamBenh/api/xemDVHNAPI.php`, {
                method: 'GET',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' }
            })
                ...
                .then(data => {
                    ...
                    const now = new Date();
                    const time = now.toTimeString().slice(0, 5); // Lấy giờ hiện tại dạng "HH:mm"
                    let html = '';
                    if (data.success) {
                                ...
                                html += `
                                    <div class="order-card bg-white rounded-lg shadow-md p-4 border border-gray-200 flex flex-col">
                                        <div class="flex-1">
                                            ...
                                            <p class="text-sm"><i class="fas fa-wallet mr-2"></i><strong>Chi phí:</strong> ${item.tongtien ? parseInt(item.tongtien).toLocaleString('vi-VN') + ' VND' : 'N/A'}</p>
                                        </div>
                                        <div class="mt-3 flex flex-row justify-center space-x-2">
                                            ...
                                            <button onclick="batDauDichVu('${item.id || ''}')" class="inline-block bg-yellow-500 text-white px-3 py-1 rounded text-sm hover:bg-yellow-600">
                                                <i class="fas fa-play mr-1"></i>Thực hiện dịch vụ
                                            </button>
                                            <button onclick="ketThucDichVu('${item.id || ''}')" class="inline-block bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600">
